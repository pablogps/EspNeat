...
public class ArtistWelderControllerCanvasAsInput : UnitController {

    private static int ID_counter = 0;
    private int ID;

    // The number of inputs, excluding bias and the pixelized canvas (this is used
    // as an offset to write these canvas inputs)-
    int inputOffset = 5;
    ...
    private const float pixelSizeZinv = 30.379747f;
    ...
    // x 10 so that inputs are more sensitive to a little writing!
    ...
    bool IsTowardsTarget(out float distance, ref ISignalArray inputArr)
    {
                            ...
                            out hit, sensor_range))
        {
            ...
            if (hit.collider.tag == "ArtCanvas")
            {
                ...
                pixelColours[PointToId(hit.point, ref inputArr)] = 1f;
                return true;
            }
        }
        else
