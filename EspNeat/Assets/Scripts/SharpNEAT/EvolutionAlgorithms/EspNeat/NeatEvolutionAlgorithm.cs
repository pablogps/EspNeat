...
namespace SharpNeat.EvolutionAlgorithms
{
    ...
    {
        // If manual is true automatic evolution will be paused
        private bool manual = false;
        // We need to access a manual evolution algorithm method
        private NeatManualEvolution<TGenome> manual_ea;        
        // This is used to know when NeatManualEvolution is done
        private bool proceed = false;
        // We need to know if the next generation will also be manual to avoid
        // running units for evaluation! We don't want to use "manual" here, 
        // because this could interfere with the normal evaluation if it is
        // activated during a normal generation (is_next_manual will skip the
        // lengthy unit trials and fitness evaluation)
        private bool is_next_manual = false;
        ...
        /// <summary>
        /// Gets or sets the algorithm's manual trigger
        /// </summary>
        public bool Manual 
        {
            get { return manual; }
        ...
        /// <summary>
        /// Receives the manual evolutionary algorithm function
        /// </summary>
        public NeatManualEvolution<TGenome> Manual_ea 
        {
            set { manual_ea = value; }
        }

		/// <summary>
		/// When the manual selection is done this allows the waiting process to end.
		/// </summary>
        public bool Proceed
        {
            set { proceed = value; }
        ...
        /// <summary>
        /// If the next evaluation is also going to be manual there is no need
        /// to create units and allow them to remain active during the trial
        /// duration for evaluation.
        /// </summary>
        public bool IsNextManual
        {
            set { is_next_manual = value; }
        }
        ...
        {
            // If this is true, there will be no offspring.
            isManualStop = false;
            ...
            _specieList = _speciationStrategy.InitializeSpeciation(_genomeList, 
                                                                   _eaParams.SpecieCount);
        ...
        {
            // If we are in manual mode we call the manual function, which will
            // modify the fitness values of the genomes. From that point evolution
            // (creation of offspring) goes on as usual. If another manual 
            // generation is intended, there is no need to have the offspring 
            // functioning for a trial period and then evaluated!
            if (manual == true) 
			{   
				// This function will spawn units for the genomes in the list. 
                // The user will then select interesting units, which receive 
                // a higher fitness value. The units are killed, but _genomeList
                // will have the updated fitness values.
				manual_ea.ManualSelection(_genomeList);

                // Waiting for the manual process to be over
                while (proceed == false)
                {
                    // This are the same waiting periods as in NeatManualEv.
                    yield return new WaitForSeconds(0.1f);
                }       
                // Do not forget to reset the "proceed" variable.
                proceed = false;
                // Just some breathing time before leaving
                yield return new WaitForSeconds(0.1f);
            ...
            // Creates offspring unless the user is trying to stop a manual
            // evolution experiment.
            // Calculates the statistics for a new generation, creates
            // the offspring and updates the statistics.
            if (!isManualStop)
            {
                // Here we advance one generation.
                _currentGeneration++;
                ...
                // Create offspring.
                List<TGenome> offspringList = CreateOffspring(specieStatsArr, 
                    offspringCount);
                ...
                // Evaluate genomes if manual mode is not selected for next generation
                if (is_next_manual == false)
                {
                    yield return Coroutiner.StartCoroutine(_genomeListEvaluator.Evaluate(_genomeList));
                    // This little delay avoids the creation of new elements before
                    // the previous batch has been destroyed (which takes very little
                    // but SOME time). This normally has no implications.
                ...
                // Sort the genomes in each specie. Fittest first (secondary sort 
                // - youngest first).
                SortSpecieGenomes();
        ...
        private SpecieStats[] CalcSpecieStats(out int offspringCount)
        {
            ...
            else if (targetSizeDeltaInt > 0)
            {
                ...
                {
                    SpecieStats inst = specieStatsArr[i];
                    probabilities[i] = Math.Max(0.0, (double)inst._targetSizeInt - 
            ...
            for (int i = 0; i < specieCount; i++)
            {
                ...
                double offspringAsexualCountReal = (double)inst._offspringCount * 
        ...
        private List<TGenome> CreateOffspring(SpecieStats[] specieStatsArr, int offspringCount)
        {
            ...
            // --- Produce the required number of offspring from sexual reproduction.
                ...
                // Test for special case - we only have one genome to select 
                // from in the current specie. 
                if (1 == inst._selectionSizeInt)
                {   
                    // Fall-back to asexual reproduction. Still each individual
                    // from the offspring will be unique!
