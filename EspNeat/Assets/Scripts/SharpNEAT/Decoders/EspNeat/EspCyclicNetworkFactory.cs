...
namespace SharpNeat.Decoders
{
    ...
    public class EspCyclicNetworkFactory
    {
                ...
                NeatGenome givenGenome, NetworkActivationScheme activationScheme)
        {
            ...
            // Gets some of the variables in countVariables.
            GetEasyVariables();
...
/*          foreach (KeyValuePair<int, int> pair in oldToNewIndex)
        ...
        private static void CreatePandemoniumArray()
        {
            ...
            // We need to count the different pandemoniumn groups there are in
            // the genome. We also count the number of regulatory neurons in
            // each pandemonium. We can do both things at the same time with a
            // dictionary.
            SortedDictionary<int, int> pandemoniumsFound = new SortedDictionary<int, int>();
            // The first regulatory neuron is in the index:
            int firstReg = genome.NeuronGeneList.LastBase - genome.Regulatory + 1;
            // (Last base is the index for the last regulatory. We need to go 
            // back "genome.Regulatory - 1" possitions).
            for (int i = firstReg; i <= genome.NeuronGeneList.LastBase; ++i)
            {
            ...
            // If the first pandemonium key is > 0 it means all modules belong
            // in a pandemonium. We need to leave the first index of the array
            // empty, because regulatory neurons here will be treated as if
            // they do NOT belong in a pandemonium!
            ...
            // Updates the number of pandemoniums in phenomeVariables.
            ...
            // second dimension of the array. This time we are only interested
            // in the value (number of regulatory neurons in this group)
            int order = 0;
            foreach (KeyValuePair<int, int> entry in pandemoniumsFound)
            {
                ...
                // After we have read this value we overwrite it with the order
                // in which we found the pandemonium value. In this way we obtain
                // a new dictionary: pandemonium value --> pandemonium order 
                // in the list.
                pandemoniumsFound[entry.Key] = order;
                ++order;
            }

            // Finally goes again through the regulatory neurons, and adds their
            // phenome index in the 2D-array.
            // We create an array so we know how many we have already 
            // added to each pandemonium group.
            int[] regInModuleCurrent = new int[pandemoniumsFound.Count];
            Array.Clear(regInModuleCurrent, 0, pandemoniumsFound.Count);

            for (int i = firstReg; i <= genome.NeuronGeneList.LastBase; ++i)
            {
                /*
                int cosa7;
                if (!idToNewIndex.TryGetValue(genome.NeuronGeneList[i].Id, out cosa7))
                {
                    UnityEngine.Debug.Log("ERROR7 " + genome.NeuronGeneList[i].Id);
                }
                */
                // pandGroup is the group to which belongs the regulatory neuron.
                ...
                int pandGroup = pandemoniumsFound[genome.NeuronGeneList[i].Pandemonium];
                // We add the index for phenomes, but this has not changed for
                // regulatory neurons with respect to the index in genomes.
                phenomeVariables.pandemonium[pandGroup][regInModuleCurrent[pandGroup]] = 
                        idToNewIndex[genome.NeuronGeneList[i].Id];
                // We increase the counter for this pandemonium group.
                ++regInModuleCurrent[pandGroup];
            }
        }
